...
from gitential2.datatypes.repositories import RepositoryStatus, RepositoryStatusPhase
...
def get_project_status(g: GitentialContext, workspace_id: int, project_id: int) -> ProjectStatus:
    ...
    project = get_project(g, workspace_id, project_id=project_id)
    repo_statuses = [get_repo_status(g, workspace_id, r.id) for r in repositories]
...
def get_repo_status(g: GitentialContext, workspace_id: int, repository_id: int) -> RepositoryStatus:
    ...
    if current_status_dict:
        ...
        if repository:
            ...
            g.kvstore.set_value(_repo_status_key(workspace_id, repository_id), initial_status.dict())
...
def update_repository_status(g: GitentialContext, workspace_id: int, repository_id: int, **kwargs) -> RepositoryStatus:
    current_status = get_repo_status(g, workspace_id, repository_id)
